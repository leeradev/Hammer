module.exports = class Command {
  constructor(client) {
    this.client = client;
    this.name = "softban";
    this.info = "Softbans a user from the server.";
    this.args = "<user>";
  }
  ...
  async run(message, args) {
    ...
    message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1, time: 200000, errors: ['time'] }).then(collected => {
      message.guild.member(message.mentions.users.first()).ban().then(member => {
        message.channel.sendMessage(`:ok_hand: Successfully softbanned **${member.user.username}** for \`${collected.first().content}\`.`);
