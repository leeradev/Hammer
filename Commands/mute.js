module.exports = class Command {
  constructor(client) {
    this.client = client;
    this.name = "mute";
    this.info = "Mutes a user from the server.";
    this.args = "<user>";
  }
  ...
  async run(message, args) {
    ...
    message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1, time: 200000, errors: ['time'] }).then(collected => {
      ...
      if(!role) return message.channel.sendMessage(`:warning: **HammerMuted** role is required.`);
      message.guild.member(message.mentions.users.first()).addRole(role.id);
      message.channel.sendMessage(`:ok_hand: Successfully muted **${message.mentions.users.first().username}** for \`${collected.first().content}\`.`);
